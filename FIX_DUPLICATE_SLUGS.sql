...
-- =====================================================
-- This updates the import function to handle ranges with duplicate names in the same city
-- by adding a unique suffix to the slug
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      range_slug := lower(regexp_replace(regexp_replace(COALESCE(NULLIF(range_record->>'post_title', ''), 'unnamed'), '[^a-zA-Z0-9\s-]', '', 'g'), '\s+', '-', 'g'));
      ...
      INSERT INTO ranges (
        ...
        range_slug,  -- Use the unique slug we generated
        city_record_id,
        province_record_id,
...
SELECT '✅ Duplicate slug handling added!' as status;
SELECT 'Now import your CSV again - all 44 ranges should import successfully!' as next_step;
