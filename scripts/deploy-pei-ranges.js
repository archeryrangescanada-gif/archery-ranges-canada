/**
 * Deploy Prince Edward Island Archery Ranges to Supabase
 * 
 * Reads prince_edward_island_archery_ranges_enriched.csv from the project root,
 * parses each row, and inserts them as live listings via the Supabase service role client.
 * 
 * Usage: node scripts/deploy-pei-ranges.js
 */
...
// ─── Main ────────────────────────────────────────────────────────────
async function main() {
    console.log('🏹 Prince Edward Island Ranges Deployment');
    console.log('──────────────────────────────────────────');

    // 1. Read CSV
    const csvPath = path.join(__dirname, '..', 'prince_edward_island_archery_ranges_enriched.csv');
    if (!fs.existsSync(csvPath)) {
        console.error('❌ prince_edward_island_archery_ranges_enriched.csv not found in project root');
        process.exit(1);
    }
    ...
    if (existingProv) {
        ...
        console.log(`✅ Province "Prince Edward Island" already exists (ID: ${provinceId})`);
    } else {
        const { data: newProv, error: provErr } = await supabase
            .from('provinces')
            .insert({ name: 'Prince Edward Island', slug: 'prince-edward-island' })
            .select('id')
            .single();

        if (provErr) {
            console.error('❌ Failed to create Prince Edward Island province:', provErr.message);
            process.exit(1);
        }
        provinceId = newProv.id;
        console.log(`✅ Created province "Prince Edward Island" (ID: ${provinceId})`);
    }
