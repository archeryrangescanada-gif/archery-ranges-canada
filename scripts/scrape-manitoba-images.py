...
CSV_FILE = 'manitoba_archery_ranges_enriched.csv'
OUTPUT_DIR = 'public/manitoba_listing_images'
...
async def main():
                ...
                try:
                    ...
                    for img in images:
                        ...
                        if src.startswith('http') and not src.endswith('.svg') and 'logo' not in src.lower() and 'icon' not in src.lower():
                            # Ideally we also want 'score' > 0 if crawl4ai provides it
                    ...
                    if best_image:
                        ...
                        if success:
                            ...
                            downloaded_mappings.append((name, f"/manitoba_listing_images/{filename}"))
                        else:
    ...
    with open('manitoba_images_mapping.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['name', 'image_path'])
        writer.writerows(downloaded_mappings)
    print("Mapping saved to manitoba_images_mapping.csv")
