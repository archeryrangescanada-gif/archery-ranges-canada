...
async function main() {
    // Read mapping file
    const realImages = {};
    if (fs.existsSync('yukon_images_mapping.csv')) {
        const lines = fs.readFileSync('yukon_images_mapping.csv', 'utf-8').split('\n').filter(l => l.trim());
    ...
    const { data: p } = await supabase.from('provinces').select('id').eq('slug', 'yukon').single();
    ...
    let real = 0, filler = 0;
    for (const r of ranges) {
        const img = realImages[r.name];
        if (img) real++; else filler++;
    }
