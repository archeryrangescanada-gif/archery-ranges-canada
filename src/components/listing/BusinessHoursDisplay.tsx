...
function isCurrentlyOpen(hours: BusinessHours, t: (key: string, params?: Record<string, string | number>) => string): { isOpen: boolean; nextChange: string | null } {
  const now = new Date();
  const currentDay = getCurrentDay();
  const dayHours = hours[currentDay];

  if (!dayHours || dayHours.closed) {
    return { isOpen: false, nextChange: t('rangePage.closedToday') };
  }
  ...
  if (currentTime < openTime) {
    return { isOpen: false, nextChange: t('rangePage.opensAt', { time: formatTime(dayHours.open) }) };
  }

  if (currentTime >= openTime && currentTime < closeTime) {
    return { isOpen: true, nextChange: t('rangePage.closesAt', { time: formatTime(dayHours.close) }) };
  }

  return { isOpen: false, nextChange: t('rangePage.closedForToday') };
}
...
export function BusinessHoursDisplay({ hours }: BusinessHoursDisplayProps) {
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-6">
        <div className="flex items-center gap-4 mb-3">
          ...
          <div>
            <h3 className="font-medium text-stone-800">{t('rangePage.businessHours')}</h3>
  ...
  const currentDay = getCurrentDay();
  const { isOpen, nextChange } = isCurrentlyOpen(hours, t);
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
          ...
          <div className="text-left">
            <div className="flex items-center gap-2">
              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium ${isOpen ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-100 text-stone-600'}`}>
                {isOpen ? t('rangePage.openNow') : t('rangePage.closed')}
              </span>
            </div>
            <p className="text-sm text-stone-500 mt-1">
              {nextChange || (todayHours?.closed ? t('rangePage.closedToday') : todayHours?.open && todayHours?.close ? `${formatTime(todayHours.open)} - ${formatTime(todayHours.close)}` : t('rangePage.hoursNotAvailable'))}
            </p>
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="border-t border-stone-200 px-6 py-4">
              ...
              return (
                <div key={day} className={`flex items-center justify-between py-2 px-3 rounded-lg ${isToday ? 'bg-emerald-50 border border-emerald-200' : ''}`}>
                  <span className={`font-medium ${isToday ? 'text-emerald-700' : 'text-stone-700'}`}>
                    {t(`rangePage.days.${day}`)}
                    {isToday && <span className="ml-2 text-xs text-emerald-600">{t('rangePage.today')}</span>}
                  </span>

                  <span className={`${dayHours?.closed ? 'text-stone-400' : isToday ? 'text-emerald-700 font-medium' : 'text-stone-600'}`}>
                    {dayHours?.closed ? t('rangePage.closed') : dayHours?.open && dayHours?.close ? `${formatTime(dayHours.open)} - ${formatTime(dayHours.close)}` : '—'}
                  </span>
                </div>
              );
            })}
          ...
          <p className="text-xs text-stone-400 mt-4 text-center">{t('rangePage.hoursVaryHolidays')}</p>
        </div>
      )}
    </div>
  );
